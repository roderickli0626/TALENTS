...
namespace TALENTS
{
    public partial class PaymentCompleteU : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            // Save Purchase Subscription Info to DB

            Model user = new LoginController().GetCurrentUserAccount();
            if (user == null)
            {
            ...
            bool result = subscriptionUController.SaveUserSubscription(user.Id, subscriptionID, false);
            UserCredit userCredit = new UserCreditDAO().FindByUser(user.Id);
            if (userCredit != null)
            {
